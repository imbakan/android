...
public class Model {
    ...
    public void create(int handle) {
        ...
        ByteBuffer bb;
        int[] vbo = new int[1];
        int k, vertex_size, color_size, vertex_offset, color_offset, location2, location3, stride, size, alloc_size;
        float[] vertices;

        vertex_size = 2; // x, y
        color_size = 3; // r, g, b

        stride = (vertex_size + color_size) * Float.BYTES;

        vertex_offset = 0;
        color_offset = vertex_size * Float.BYTES;;

        count = 3;
        size = count * stride;
        alloc_size = count * (vertex_size + color_size);
        ...
        location3 = GLES32.glGetAttribLocation(handle, "v_color");
        ...
        GLES32.glVertexAttribPointer(location3, color_size, GLES32.GL_FLOAT, false, stride, color_offset);
        GLES32.glEnableVertexAttribArray(location3);
